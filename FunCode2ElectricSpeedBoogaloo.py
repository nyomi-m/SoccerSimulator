...
##############################################SPEED TRACKING################################################
            if ballStopped is True:
                if minX is 0 and (x_pos[len(x_pos)-1] < 150):
                    minX = x_pos[len(x_pos)-1]
                if (x_pos[len(x_pos)-1] < minX-24):
                    print('Movement, frame counter starting')
                    ballStopped = False
            if ballStopped is False:
                if(x_pos[len(x_pos)-1] < minX):
                    minX =  x_pos[len(x_pos)-1]
                frameCounter = frameCounter + 1
                print(x_pos[len(x_pos)-1], '         ', minX)
                if x_pos[len(x_pos)-1] > minX+8:
                    print('Wall hit, stopping counter')
                    ballStopped = True
...
vs.release()
##########################################Speed Calculation######################################
print('Speed: ', 20.5/(frameCounter/30), '   Frames: ', frameCounter)
#################################################################################################
# Close program:
